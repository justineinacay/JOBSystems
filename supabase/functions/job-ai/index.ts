const APP_ORIGIN = "https://justineinacay.github.io";
...
const allowedOrigin = (request: Request) => {
  ...
  const origins = new Set([APP_ORIGIN, ...configuredOrigins()]);
  return origins.has(origin) ? origin : APP_ORIGIN;
};
...
Deno.serve(async (request) => {
  ...
  if (!supabaseUrl || !anonKey || !openaiKey || !model) return json({ error: "J.E.L.I.X. is not configured." }, 503, origin);
