(function(){
  ...
  function chartMarkup(){
    var data=trend(),points=data.values.map(function(value,index){var x=20+index*43;var y=90-(value/data.max*60);return x+','+y;}).join(' ');
    return '<div class="health-game-chart-wrap"><svg class="health-game-chart" viewBox="0 0 280 112" role="img" aria-label="Seven-day health activity trend"><path class="health-game-grid" d="M20 30H278M20 60H278M20 90H278"></path><polyline points="'+points+'"></polyline>'+data.values.map(function(value,index){var x=20+index*43;var y=90-(value/data.max*60);return '<circle cx="'+x+'" cy="'+y+'" r="3"></circle>';}).join('')+'</svg><div class="health-game-chart-labels">'+data.days.map(function(day){return '<span>'+dateObj(day).toLocaleDateString('en-PH',{weekday:'short'}).slice(0,2)+'</span>';}).join('')+'</div></div>';
  }
  ...
  function render(){
    ...
    mount.innerHTML='<div class="health-game-header"><div><span class="health-game-kicker">PERSONAL HEALTH SYSTEM</span><h2>Fitness Game Dashboard</h2><p>Track manually. Build consistency. Win progress.</p></div><div class="health-game-head-stats"><span><i class="ti ti-flame"></i><strong>'+streakDays+'</strong><small>Day streak</small></span><span><i class="ti ti-diamond"></i><strong>'+xp.xp+'</strong><small>Total XP</small></span><button class="health-game-log" onclick="openModal(\'healthLogModal\')"><i class="ti ti-plus"></i>Log health data</button></div></div>'+
      '<div class="health-game-grid">'+
        ...
        '<article class="health-game-card health-game-challenge-card"><div class="health-game-card-title"><span><i class="ti ti-calendar-week"></i>Weekly challenge</span><small>5-day target</small></div><p class="health-game-card-subtitle">Log meaningful health data on different days.</p><div class="health-game-week-strip">'+weekDates().slice(0,7).map(function(day){var active=!!exerciseDays()[day];return '<span class="'+(active?'is-active':'')+'">'+dayLabel(day).slice(0,1)+'</span>';}).join('')+'</div><div class="health-game-challenge-row"><strong>'+Math.min(5,weekly)+' / 5 days completed</strong><span>'+Math.min(100,Math.round(weekly/5*100))+'%</span></div><div class="health-game-xp-bar"><i style="width:'+Math.min(100,weekly/5*100)+'%"></i></div><div class="health-game-achievement"><i class="ti ti-medal-2"></i><span><small>Next achievement</small><b>'+ (weekly>=5?'Perfect week':'Momentum builder') +'</b></span><strong>'+ (weekly>=5?'CLAIMED':'+'+(5-weekly)+' day'+(5-weekly===1?'':'s')) +'</strong></div></article>'+
